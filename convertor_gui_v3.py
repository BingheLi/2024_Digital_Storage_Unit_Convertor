...
class StorageConverter:
    ...
    def __init__(self):
        ...
        self.storage_instructions = Label(self.storage_frame, text=instructions, wrap=250, width=40, justify="left", bg="#FFE4E1", fg="#FF69B4")
        self.storage_instructions.grid(row=1, columnspan=4, pady=(0, 10))

        # Entry field for the user input
        self.value_entry = Entry(self.storage_frame, font=("Arial", "14"))
        ...
        self.from_unit_dropdown = OptionMenu(self.storage_frame, self.from_unit_var, "bytes", "kilobytes", "megabytes", "gigabytes")
        ...
        self.to_unit_dropdown = OptionMenu(self.storage_frame, self.to_unit_var, "bytes", "kilobytes", "megabytes", "gigabytes")
        self.to_unit_dropdown.grid(row=4, column=1, padx=5, pady=5, sticky=W)

        # Label for displaying output or error messages
        error = "Please enter a number"
        self.output_label = Label(self.storage_frame, text="", fg="#FF69B4", bg="#FFE4E1")
        self.output_label.grid(row=5, columnspan=4)

        # Buttons
        button_bg = "#FF69B4"
        self.history_button = Button(self.storage_frame, text="Export/History", bg="#FFC0CB", fg="#000000", font=("Arial", "12", "bold"), width=12)
        self.history_button.grid(row=7, column=2, columnspan=2, padx=5, pady=5)

        self.clear_button = Button(self.storage_frame, text="Clear", bg=button_bg, fg="#FFFFFF", font=("Arial", "12", "bold"), width=12)
        self.clear_button.grid(row=2, column=2, padx=5, pady=5)

        self.convert_button = Button(self.storage_frame, text="Convert", bg=button_bg, fg="#FFFFFF", font=("Arial", "12", "bold"), width=12)
        self.convert_button.grid(row=6, columnspan=1, padx=5, pady=5)

        self.solution_button = Button(self.storage_frame, text="Show Solution", bg=button_bg, fg="#FFFFFF", font=("Arial", "12", "bold"), width=12)
        self.solution_button.grid(row=6, column=2, columnspan=2, padx=5, pady=5)

        self.donation_button = Button(self.storage_frame, text="Donate", bg="#FFC0CB", fg="#000000", font=("Arial", "12", "bold"), width=12)
